...
public class QRScannerOverlay extends View {
    ...
    public QRScannerOverlay(Context context, AttributeSet attrs) {
        super(context, attrs);
        scanSize = getResources().getDisplayMetrics().density * 270f;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float left = centerX - scanSize / 2f;
        float top = centerY - scanSize / 2f;
        float right = centerX + scanSize / 2f;
        float bottom = centerY + scanSize / 2f;
        RectF scanRect = new RectF(left, top, right, bottom);

        canvas.drawRect(0, 0, getWidth(), getHeight(), overlayPaint);
        canvas.drawRoundRect(scanRect, 18f, 18f, clearPaint);
    }
}